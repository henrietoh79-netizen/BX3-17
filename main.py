...
def verify_token(x_bx3_token: str = Header(None)):
    # 🔓 Désactivé pour compatibilité APK
    return True
...
def _compute_vadigo(fixture_id: str, fix_data: dict) -> AnalysisResponse:
    rng = random.Random(int(fixture_id.replace("demo-","").replace("-","")[:8] or "42", 16) if all(c in "0123456789abcdef" for c in fixture_id[:8].lower()) else hash(fixture_id) % 10000)
